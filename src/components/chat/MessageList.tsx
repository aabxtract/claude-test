...
export function MessageList({ messages, isLoading }: MessageListProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto px-4 py-6 bg-slate-900">
          ...
          >
            {message.role === "assistant" && (
              <div className="flex-shrink-0">
                <div className="w-9 h-9 rounded-lg bg-slate-800 border border-slate-700 shadow-sm flex items-center justify-center">
                    ...
                    <>
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "reasoning":
                            return (
                              <div key={partIndex} className="mt-3 p-3 bg-slate-700/50 rounded-md border border-slate-600">
                                <span className="text-xs font-medium text-slate-300 block mb-1">Reasoning</span>
                                <span className="text-sm text-slate-200">{part.reasoning}</span>
                              </div>
                            );
                          case "tool-invocation":
                            const tool = part.toolInvocation;
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-slate-700 rounded-lg text-xs font-mono border border-slate-600">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                                    <span className="text-slate-200">{tool.toolName}</span>
                                  </>
                                ) : (
                                  <>
                          ...
                          case "step-start":
                            return partIndex > 0 ? <hr key={partIndex} className="my-3 border-slate-700" /> : null;
                          default:
                            return null;
                        }
                      })}
